...
> **For BUYER projects only.** This skill is what you install when you're building an agent that consumes Loomal — inbox, vault, calendar, identity, paying for x402-priced APIs. If you're shipping a paid endpoint, you don't need this skill — wire up [`@loomal/sdk/paywall/*`](https://docs.loomal.ai/docs/for-seller/accept-payments) middleware in your own server instead.
...
- **Payments** — pay any x402-priced URL in USDC on Base under per-call + daily caps you set as a mandate
...
Get a `loid-...` key for a **BUYER project** at [console.loomal.ai](https://console.loomal.ai). The MCP server fetches `/v0/whoami` at startup and only registers tools matching the key's scopes, so a SELLER key plugged in here will surface almost nothing.
...
It should call `identity_whoami` and return the agent's email (`agent-xxx@mailgent.dev`), DID, scopes, and `purpose` (`BUYER`).
...
> Show me what I've spent on paid APIs this week
```
...
| `mail_*` | `send`, `reply`, `list_messages`, `get_message`, `get_attachment`, `list_threads`, `get_thread`, `update_labels`, `update_thread_labels`, `delete_message`, `delete_thread`, `list_rules`, `add_rule`, `delete_rule` |
| `vault_*` | `list`, `get`, `store`, `delete`, `totp`, `totp_use_backup` |
| `calendar_*` | `list`, `get`, `create`, `update`, `delete`, `set_public` |
| `payments_*` | `pay`, `activity`, `mandates_create`, `mandates_list`, `mandates_get`, `mandates_revoke` |

Tools only appear if your identity has the required scopes (`mail:send`, `vault:write`, `payments:spend`, etc.). Add scopes at [console.loomal.ai](https://console.loomal.ai).
...
```bash
openclaw mcp set loomal-sales '{"command":"npx","args":["-y","@loomal/mcp@0.6.1"],"env":{"LOOMAL_API_KEY":"loid-sales-key"}}'
openclaw mcp set loomal-support '{"command":"npx","args":["-y","@loomal/mcp@0.6.1"],"env":{"LOOMAL_API_KEY":"loid-support-key"}}'
```
